...
class GameViewModel(application: Application) : AndroidViewModel(application) {
        ...
        viewModelScope.launch {
            while (true) {
                ...
                _gameState.update { currentState ->
                    ...
                    val goldPerSecond = currentState.facilities[FacilityType.RESEARCH_WING]?.level?.let { BigDecimal(it) } ?: BigDecimal.ZERO
                    currentState.copy(
    ...
    fun upgradeDepartment(type: DepartmentType) {
        _gameState.update { currentState ->
            ...
            val newTotalMagicalPower = calculateTotalMagicalPower(currentDepartments, currentState.facilities, currentState.students)
    ...
    fun upgradeFacility(type: FacilityType) {
        _gameState.update { currentState ->
            ...
            val manaCost = BigDecimal(20).pow(facilityState.level)
            var goldCost = BigDecimal.ZERO

            if (type == FacilityType.DIMENSIONAL_LIBRARY) {
                goldCost = BigDecimal(100).pow(facilityState.level)
            }

            if (currentState.mana < manaCost || currentState.gold < goldCost) {
                return@update currentState
            }

            val newMana = currentState.mana.subtract(manaCost)
            val newGold = currentState.gold.subtract(goldCost)
            ...
            val newTotalMagicalPower = calculateTotalMagicalPower(currentState.departments, currentFacilities, currentState.students)
    ...
    fun recruitStudent() {
        _gameState.update { currentState ->
            val cost = BigDecimal(100).multiply(BigDecimal(currentState.students.totalStudents + 1))
            ...
            val newTotalMagicalPower = calculateTotalMagicalPower(currentState.departments, currentState.facilities, newStudents)
    ...
    private fun calculateTotalMagicalPower(departments: Map<DepartmentType, DepartmentState>, facilities: Map<FacilityType, FacilityState>, students: StudentState): BigDecimal {
        val totalDepartmentLevel = departments.values.sumOf { it.level }
        val totalFacilityLevel = facilities.values.sumOf { it.level }
        val studentBonus = students.totalStudents
        return BigDecimal((totalDepartmentLevel + totalFacilityLevel) * 10 + studentBonus)
    }
